...
export default class DonationHttpService {
  ...
  static async fetchUserCollections(userId) {
    try {
      const res = await axios.get(
        `${this.rootUrl}/collections/${userId}/index`
      );
      return res.data;
    } catch (err) {
      return null;
  ...
  static async updateUserDrive(userId, books) {
    try {
      const res = await axios.put(
        `${this.rootUrl}/users/${userId}/drive`,
        books
      );
      return res.data;
    } catch (err) {
  ...
  static async updateCollection({ books, points, status, collectionId }) {
    try {
      const res = await axios.put(
        `${this.rootUrl}/collections/${collectionId}`,
        { books, points, status }
      );
      return res.data;
    } catch (err) {
  ...
  static async fetchNearestBookCollectors({ lat, long }) {
    try {
      const res = await axios.post(
        `${this.rootUrl}/users/index?long=${long}&latt=${lat}`
      );
      return res.data;
    } catch (err) {
      return null;
  ...
  static async getBookInfo(isbn) {
    try {
      const res = await axios.get(
        `${this.googleBooksUrl}/volumes?q=isbn:${isbn}&key=${this.googleBooksApiKey}`
      );
      return res.data;
    } catch (err) {
      return null;
