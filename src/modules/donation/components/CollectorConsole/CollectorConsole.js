...
class CollectorConsole extends Component {
  ...
  handlePatchCollection = async (c_id, queued) => {
    ...
    const updatedCollection = await DonationHttpService.updateCollection({
      books: newData.books,
  ...
  render() {
    ...
    const beginView = (
      <Container maxWidth="sm">
        ...
        >
          ...
          >
            <Typography>
              Start scanning QR Codes to add to your book drive!
            </Typography>
          </Box>
          <Box
            display="flex"
      ...
      <>
        <DonationContext.Provider value={userContextVal}>
          <QRScanner
            ...
            onAddCollection={(c) => this.handleAddCollection(c)}
          />
          <Box
            display="flex"
            flexDirection="row"
    ...
    if (this.state.currentPhase !== -1 && this.state.currentPhase !== 0) {
      ...
      currentView =
        this.state.currentPhase === 0 ? qrScannerView : barcodeScannerView;
    }

    const currentStatus =
      !this.state.books.length && this.state.currentPhase !== 0
        ? "Welcome to Kitabu!"
        : !this.state.books.length && this.state.currentPhase !== -1
        ? "Welcome to Kitabu!"
        : this.state.currentPhase === 1
        ? "Host Book Drives for Local Donors"
        : this.state.currentPhase === 2
        ? "Package Your Collection"
        : null;
