  ...
  getBookInfo = async (isbn) => {
    ...
    if (!googleBookData) {
      ...
      if (googleBookData.totalItems !== 0) {
        ...
        serializedBookData.thumbnail = !bookData.volumeInfo.imageLinks
          ?.thumbnail
          ? ""
  ...
  render() {
    const message = this.state.error ? (
      ...
      <Alert severity="success">
        Book found! {this.state.scanned[0].title}
      </Alert>
    ) : null;
    ...
    return (
      <Container maxWidth="lg">
        {!this.state.manualInput ? (
          <Box minHeight="30vh" maxWidth="500px" mx="auto" p={4.44}>
        ...
        <Box display="flex" alignItems="center" justifyContent="center">
          {this.state.scanned.length ? (
            <BookItem
              ...
              handleSelectConditionScanner={(e) =>
                this.handleSelectConditionScanner(e)
              }
            />
          ) : !this.state.manualInput ? (
            <Button onClick={(e) => this.setManualInput()}>Manual Input</Button>
          ) : null}
          {this.state.manualInput ? (
            <>
              <Box display="flex" flexDirection="column">
                ...
                <Typography variant="h2">
                  Author (Separate multiple authors by using ',')
                </Typography>
                <TextField
                  label="ISBN"
                  id="isbn"
                ...
                >
                  ...
                  <Button
                    className="alternate-box--btn"
                    onClick={(e) => this.closeManualInput()}
                  >
                    Use the Scanner?
                  </Button>
                </Box>
